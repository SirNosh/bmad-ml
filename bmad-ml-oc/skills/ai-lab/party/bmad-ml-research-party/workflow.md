...
In OpenCode, spawn each selected research agent as a parallel Task tool call. Each agent runs in an isolated session, loads its own skill, and returns a structured response.

Dispatch all selected agents simultaneously in a single response:

```
Task(subagent_type="{agent}", description="{agent} on {topic}",
     prompt="Research Party context:
     Format: {format}. Central question: {question}.
     Context summary: {context_summary}.
     Artifacts to read: {artifact_paths}.
     Your role: Provide your specialist perspective on this question.
     Return: position, supporting evidence, concerns, and recommendations.")
```

Available research agent `subagent_type` values: `sova`, `sage`, `cypher`, `viper`, `breach`, `fade`, `astra`, `killjoy`.

Collect all agent responses, then proceed to synthesis (agreement/tension mapping, recommendations).

For follow-up rounds, dispatch a second parallel batch with prior responses included in the prompt.
