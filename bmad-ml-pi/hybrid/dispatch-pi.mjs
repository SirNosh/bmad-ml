...
function resolveModel({ cwd, agent, cliModel, manifestModel }) {
  if (cliModel) {
    const parts = cliModel.split(":");
    if (parts.length === 2) {
      return { provider: parts[0], id: parts[1], source: "cli" };
    }
  }
  ...
  if (fs.existsSync(settingsPath)) {
    try {
      ...
      const key = agent.replace(/^bmad-ml-/, "").replace(/^bmad-/, "");
      const setting = settings?.bmad_ml?.models?.[key];
  ...
  return {
    ...
    reasoning: process.env.PI_REASONING,
    source: "env",
  };
}
