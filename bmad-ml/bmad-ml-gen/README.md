...
BMad ML is organized around 21 agent personas across two divisions, each with their own expertise, principles, and a small set of capabilities they know how to run. **Agents are the primary interface** -- you invoke `bmad-ml-sova` when you want literature surveyed, `bmad-ml-dumbledore` when you want an AI product architected, `bmad-ml-jett` when you want experiment code shipped. Workflows exist, but they are the tools agents use; you don't call them yourself unless you want to.
...
`ml-setup` offers to create IDE project bootstrap files (also written by `npx bmad-ml --cur` / `--cc`):

- **Claude Code**: `CLAUDE.md` plus `.claude/rules/bmad-ml-nosh.md`. The `--cc` install also merges `"agent": "bmad-ml-nosh"` and hook entries into `.claude/settings.json`.
- **Cursor**: `AGENTS.md` plus `.cursor/rules/bmad-ml-nosh.mdc` and `.cursor/hooks.json`.
...
2. Start a new chat -- `AGENTS.md` bootstraps Nosh as the main-chat persona
3. Describe the outcome you need. Nosh routes work and delegates to specialists via the Task tool with `subagent_type: "bmad-<name>"`. Each specialist runs in a fresh context window and returns a structured summary.
4. You can also @-mention a specialist directly (e.g. `@bmad-sova`) to invoke them without going through Nosh.

### In Claude Code

1. Open your project in Claude Code
2. The `"agent": "bmad-ml-nosh"` setting in `.claude/settings.json` makes Nosh the session-default agent -- you'll see `@bmad-ml-nosh` in the startup header.
3. Describe the outcome you need. Nosh routes work and delegates to specialists via the Agent tool (aliased from Task in v2.1.63+) with `subagent_type: "bmad-<name>"`.
4. To opt out of the hard binding and treat Nosh as just another agent, remove the `"agent"` key from `.claude/settings.json` after install.
