...
After confirming config, detect which IDE(s) the project uses:
- If `{project-root}/.claude/` exists:
  - Offer to create or append `{project-root}/CLAUDE.md` using `./assets/claude-md-snippet.md`, resolving config tokens
  - Offer to create `{project-root}/.claude/rules/bmad-ml.md` using `./assets/cursorrules-snippet.md` content adapted to Claude rule format
- If `{project-root}/.cursor/` exists:
  - Offer to create or append `{project-root}/AGENTS.md` using `./assets/agents-md-snippet.md`, resolving config tokens
  - Offer to create `{project-root}/.cursor/rules/bmad-ml.mdc` using `./assets/cursorrules-snippet.md`
- If `{project-root}/.opencode/` exists → offer to create or append `{project-root}/AGENTS.md` using `./assets/agents-md-snippet.md`, resolving config tokens
- If multiple IDE directories exist, offer all relevant bootstrap files
- If a target already contains a managed `bmad-ml` block, skip with a note that it is already configured
