...
In OpenCode, run the roundtable as **sequential** Task tool calls. Each agent sees the full accumulated transcript from prior turns - not parallel isolated dispatch.

The facilitator selects 2-3 agents per turn based on topic relevance, then dispatches them one at a time:

```
transcript = ""

# For each turn, dispatch one agent with the full transcript:
Task(subagent_type="{agent}", description="{agent} responding to roundtable",
     prompt="Research Party roundtable context:
     Format: {format}. Central question: {question}.
     ...
     Your role: Respond as {agent} to the discussion above. Take a position,
     cite evidence, and explicitly engage with what prior speakers said.
     Return: your position, supporting evidence, where you agree or disagree
     with prior speakers, concerns, and recommendations.")

# Append each agent's response to the transcript before the next dispatch.
```

Available research agent `subagent_type` values: `sova`, `sage`, `cypher`, `viper`, `breach`, `fade`, `astra`, `killjoy`.

The facilitator analyzes the transcript after each batch of 2-3 turns, then selects the next speakers based on emerging tensions or gaps.

For follow-up rounds, continue appending to the same transcript.
