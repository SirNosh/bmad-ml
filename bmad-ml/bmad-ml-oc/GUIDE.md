...
1. Install the module skills into your IDE skills directory (`.opencode/skills/`, `.claude/skills/`, or `.cursor/skills/`)
2. **(OpenCode -- recommended)** Copy agent shims: `cp agents/opencode/*.md .opencode/agents/` -- this enables parallel subagent execution and context isolation
3. Invoke the `ml-setup` skill and answer its prompts
4. `ml-setup` writes `_bmad/config.yaml` (shared) and `_bmad/config.user.yaml` (personal)
5. Optionally, accept the prompt to append a bmad-ml section to `AGENTS.md`

After setup, you are ready to talk to the module. In OpenCode, switch to the **Nosh** agent via Tab key to start.
...
## Parallelization (OpenCode)

In OpenCode, Nosh delegates to specialist agents via the **Task tool**. Each Task call creates an isolated subagent session with its own context window. When tasks are independent, Nosh dispatches multiple Task calls in a single response -- OpenCode runs them concurrently.

### How it works

1. Nosh identifies independent tasks in the current phase
2. Nosh issues multiple Task tool calls in one response, each with `subagent_type`, `description`, and `prompt`
3. Each subagent runs in its own session: loads its skill, reads artifacts from disk, does its work, writes output
4. All subagents return structured summaries to Nosh
5. Nosh synthesizes results and decides the next phase

### Parallel windows

These phases contain independent tasks that Nosh dispatches simultaneously:

**AI Lab research phase:**
Sova (literature) + Cypher (data quality) + Viper (robustness) + Killjoy (compute) + Fade (frontier trends) -- all researching independent aspects of the same problem.

**AI Lab review phase:**
Omen (code review) + KAY/O (claim review) -- both reviewing the same experiment from different angles.

**AI Startup design phase (after architecture):**
Luna (prompt engineering) + Hagrid (data integration) -- both working from the architecture doc independently.

**AI Startup evaluation phase:**
Moody (QA evaluation) + Snape (safety audit) -- both evaluating the same system.

**Multi-agent meetings:**
All selected agents receive the agenda simultaneously and return their perspectives in parallel. The moderator synthesizes after collection.

### Sequential dependencies

These must run in order (each depends on the previous output):
- Research --> Breach (experiment design) --> Chamber (architecture) --> Jett (implementation)
- Dumbledore (product brief) --> Dumbledore (architecture) --> Dumbledore (sprint planning) --> Hermione (build) --> McGonagall (deploy)

### Fallback for non-OpenCode IDEs

In Claude Code and Cursor, the solo fallback mechanism activates: the orchestrator adopts each agent's persona sequentially within a single conversation. This produces the same artifacts but without context isolation or parallelism.
...
BMad ML skills use the standard SKILL.md format that works across all skills-capable IDEs. **OpenCode is the primary target** with full parallel subagent support. Claude Code and Cursor are supported in sequential/solo-fallback mode.

### Skill Loading
- Skills are loaded on-demand via the `skill` tool (not always-on)
- Each skill directory contains a SKILL.md with the full agent or workflow definition
- The same skill files work across OpenCode, Claude Code, and Cursor
- In OpenCode, subagents call `skill({ name: "bmad-ml-sova" })` to load their persona

### Project Instructions
During setup, ml-setup offers to create `AGENTS.md` at the project root. OpenCode auto-loads it every session (falling back to `CLAUDE.md` if absent). It tells OpenCode about the ML module's presence, entry point, and conventions.

### OpenCode Agents (Recommended)

OpenCode supports custom agents as markdown files in `.opencode/agents/`. BMad ML provides agent shims for **all 21 personas**:

- **Nosh** (`nosh.md`) -- primary agent (Tab-switchable), orchestrates via Task tool delegation
- **AI Lab Research** (8 subagents): `sova`, `sage`, `cypher`, `viper`, `breach`, `fade`, `astra`, `killjoy`
- **AI Lab Build** (5 subagents): `chamber`, `jett`, `gekko`, `omen`, `kayo`
- **AI Startup** (7 subagents): `dumbledore`, `hermione`, `snape`, `luna`, `mcgonagall`, `moody`, `hagrid`

Install: `cp agents/opencode/*.md .opencode/agents/`

Each subagent shim:
- Loads its corresponding skill via the `skill` tool
- Has `permission.skill` allowing all `bmad-ml-*` skills
- Returns structured summaries (agent, task, artifact, outcome, follow_up) so Nosh can make routing decisions
- Runs in an isolated session with its own context window

**Without agent shims**, skills still work in a single-session persona-swap mode. **With agent shims**, Nosh dispatches multiple specialists via the Task tool concurrently -- research phases, reviews, and meetings all benefit from parallelism.
