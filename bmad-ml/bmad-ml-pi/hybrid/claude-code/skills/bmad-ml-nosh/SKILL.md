...
1. Nosh emits `Task(subagent_type: "bmad-sova", prompt: "...")`.
2. The shim at `.claude/agents/bmad-sova.md` writes the prompt to `.bmad-ml/tmp/prompt-sova-<ts>.txt` and runs `node .bmad-ml/dispatch-pi.mjs sova <prompt-file>`.
...
### Task tool contract (same as sub mode)

```
Task(
  description: "3-5 word label",
  prompt: "full context: project, framework, artifacts, directive, output path",
...
Task(
  ...
  prompt: "Project: {project_name}. Survey LoRA vs full fine-tuning literature. Output: .claude/artifacts/literature-review-lora.md"
)
```
